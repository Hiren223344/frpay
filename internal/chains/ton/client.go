...
// client wraps toncenter's v3 REST API with fallback across a configured
// list of endpoints and exponential backoff.
type client struct {
...
func newClient(primary string, fallbacks []string, apiKey string) *client {
	return &client{
		baseURLs: append([]string{primary}, fallbacks...),
		apiKey:   apiKey,
		httpClient: &http.Client{
...
func (c *client) get(ctx context.Context, path string, query url.Values, out interface{}) error {
	...
	return fmt.Errorf("all toncenter endpoints failed: %w", lastErr)
}
